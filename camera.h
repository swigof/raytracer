...
class camera {
  ...
  private:
    ...
    std::mutex *color_acc_mutex;  // Mutex lock for pixel color accumulator
    color *color_acc;             // Cross-thread color accumulator
    ...
    void render_portion(shared_ptr<const hittable> world, shared_ptr<const hittable> lights,
        ...
        for (int j = start_line; j < start_line + line_count; j++) {
            for (int i = 0; i < image_width; i++) {
                color_acc_mutex->lock();
                *color_acc = color(0,0,0);
                color_acc_mutex->unlock();

                std::vector<std::thread> threads;
                for (int i = 0; i < thread_count - 1; i++) {
                    auto s_jstart = i * stratified_rows_per_thread;
                    threads.emplace_back(&camera::sample_pixel, this,
                ...
                color_acc_mutex->lock();
                image_data[j*image_width+i] = get_color(pixel_samples_scale *
                                                        *color_acc);
                color_acc_mutex->unlock();
    ...
    void sample_pixel(shared_ptr<const hittable> world, shared_ptr<const hittable> lights,
                       int i, int j, int s_jstart, int s_jcount) {
